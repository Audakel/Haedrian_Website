...
def _create_wallet(user, password):
    # TODO: fix hard coded only creation of coins.ph wallets
    wallet = CoinsPhWallet(user)
    wallet.create_wallet(user, password=password)

def new_user(data):
    if not data.get('password2'):
        data['password2'] = data.get('password1')
    _create_account(data)
    user = get_user_model().objects.get(username=data['username'])
    token = Token.objects.create(user=user)
    # If the wallet fails to create, we need to delete this user
    try:
        _create_wallet(user, data.get('password1'))
    except Exception as e:
        get_user_model().objects.get(username=data['username']).delete()
        raise e
    return {
        "token": token.key,
    }
...
def create_account(request):
    if request.method == 'POST':
        ...
        try:
            new_user(copy.deepcopy(request.POST))
